...
//获取点ch在 邻接表头数组 的索引 
//用static int ？？
static int getposition(LGraph g, char ch) { //静态函数
	int i;
...
//创建有向图的邻接表结构
LGraph* creat_example_lgraph_directed() {
	char c1, c2;
	...
	int vlen = LENGTH(vexs); //宏函数：求定点数
	int elen = LENGTH(edges); //宏函数：求边数
	int i, p1, p2;
...
int main() {
	LGraph* pG;
	//1.有向图创建
	//pG = creat_example_lgraph_directed();
	//无向图的创建
